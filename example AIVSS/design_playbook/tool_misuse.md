# วิธีคิด (Reasoning) — AI Treasury Dealing Assistant

วิธีคิด: tool_misuse ถูกขยายด้วย autonomy + tools + language (RISK_FACTOR_MATRIX) ระบบนี้ตรงทั้ง 3 ข้อพอดี — ยิง order เอง (autonomy=1), เรียกเครื่องมือซื้อขายได้หลายตัว ซึ่งเป็น external tool control surface ที่กว้าง (tools=1), และรับคำสั่งทั้งหมดผ่านภาษา ธรรมชาติจาก trader (language=1) — ช่องทางนี้เองที่เปิดให้เกิด tool squatting (เครื่องมือปลอมแอบอ้างเป็น market-data provider จริง) หรือ metadata injection ที่หลอกให้ agent เรียก tool ผิดขอบเขต

---

# AIVSS Design Review — AI Treasury Dealing Assistant
role: AI Security Lead | schema: rag.aivss-design-review-skills.v1

**Objective:** ทบทวนการออกแบบ 'AI Treasury Dealing Assistant' เทียบกับ AIVSS core risks ก่อน/ระหว่างพัฒนา ในมุมมองของ AI Security Lead (proactive design-time review)
**Scope:** รับคำสั่งจาก trader ผ่าน chat/เสียงเป็นภาษาธรรมชาติ แล้วเรียกใช้เครื่องมือซื้อขาย หลายตัวโดยอัตโนมัติ (FX pricing API, bond trading API, market-data MCP server) เพื่อยิง order เข้าตลาดโดยไม่ต้องรอ trader ยืนยันซ้ำ
**Regulatory context:** BOT market conduct / dealing room guideline, SEC Thailand, PDPA
